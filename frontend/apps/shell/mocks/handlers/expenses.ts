  ...
  http.post<never, CreateExpenseRequest, ExpenseResponse>(
    ...
    async ({ request }) => {
      ...
      const expense: Expense = {
        ...
        reportingCurrencyCode: rest.transactionCurrencyCode,
        status: "active",
        isProRata: false,
  ...
  http.post<{ id: string }, CorrectExpenseRequest, ExpenseResponse>(
    ...
    async ({ request, params }) => {
      ...
      const base: Expense = original ?? {
        ...
        userId: currentMockUser.id,
        name: rest.name,
        transactionCurrencyCode: currentMockUser.currency,
        originalTransactionAmountInMinorUnits: amount,
        reportingAmountInMinorUnits: amount,
        reportingCurrencyCode: currentMockUser.currency,
        expenseType: rest.expenseType,
        tagId: rest.tagId,
        expenseDateIso: rest.expenseDateIso,
