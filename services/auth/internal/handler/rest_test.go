...
func TestDeleteUserHandler_Success(t *testing.T) {
	...
	pwdSvc := service.NewPasswordService(4)
	hash, _ := pwdSvc.HashPassword("AdminPass1")

	repo.On("GetUserByID", mock.Anything, "admin-1").Return(&model.User{
		ID:           "admin-1",
		Username:     "admin",
	...
	repo.On("GetUserByID", mock.Anything, "user-1").Return(&model.User{
		ID:       "user-1",
		Username: "alice",
	...
	w := doJSONWithUserID(r, "DELETE", "/api/admin/users/user-1", "admin-1", map[string]string{
		"password": "AdminPass1",
	})
...
func TestDeleteUserHandler_MissingUserID(t *testing.T) {
	repo := new(mockUserRepository)
	r := setupTestRouter(repo)

	w := doJSON(r, "DELETE", "/api/admin/users/user-1", map[string]string{
		"password": "AdminPass1",
	})
...
func TestDeleteUserHandler_MissingBody(t *testing.T) {
	repo := new(mockUserRepository)
	r := setupTestRouter(repo)

	w := doJSONWithUserID(r, "DELETE", "/api/admin/users/user-1", "admin-1", nil)
...
func TestDeleteUserHandler_WrongPassword(t *testing.T) {
	repo := new(mockUserRepository)
	r := setupTestRouter(repo)

	pwdSvc := service.NewPasswordService(4)
	hash, _ := pwdSvc.HashPassword("CorrectPass1")
...
func TestDeleteUserHandler_ProtectedUser(t *testing.T) {
	repo := new(mockUserRepository)
	r := setupTestRouter(repo)
	...
	repo.On("GetUserByID", mock.Anything, "admin-2").Return(&model.User{
		ID:       "admin-2",
		Username: "admin",
		Email:    "admin@gofin.local",
		Role:     "admin",
	...
	w := doJSONWithUserID(r, "DELETE", "/api/admin/users/admin-2", "admin-1", map[string]string{
		"password": "AdminPass1",
	})
	...
	var errResp model.ApiError
	require.NoError(t, json.Unmarshal(w.Body.Bytes(), &errResp))
	assert.Equal(t, model.ErrProtectedUser, errResp.Code)
}
...
func TestDeleteUserHandler_SelfDeletion(t *testing.T) {
	...
	w := doJSONWithUserID(r, "DELETE", "/api/admin/users/admin-1", "admin-1", map[string]string{
		"password": "AdminPass1",
	})
