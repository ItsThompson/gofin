...
// Registry in both directions: no ad-hoc route escapes the Registry, and every
// auth Registry entry is bound to a handler. Adding an unclassified route, or a
// Registry entry with no handler, fails here in auth's own module (which the CI
// test-backend job runs), pointing at the Registry.
func TestRegisterRoutes_MatchesRegistry(t *testing.T) {
	...
	if err := access.VerifyRegistration("auth", registered); err != nil {
		t.Fatal(err)
	}
}
