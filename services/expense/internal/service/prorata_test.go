...
// --- Legacy migration application tests ---

func TestCreateProRataInstallment_LegacyMigrationSameCurrencyWritesMigrationSnapshot(t *testing.T) {
	repo := new(mockExpenseRepository)
	periodClient := new(mockPeriodContextClient)
	fxClient := new(mockFxClient)
	...
	req.CapturedRateSnapshot = nil
	req.LegacyMigration = true

	var captured *model.Expense
	repo.On("CreateExpense", mock.Anything, mock.AnythingOfType("*model.Expense")).
		Run(func(args mock.Arguments) {
			captured = args.Get(1).(*model.Expense)
		}).Return(&model.Expense{ID: "exp-legacy", UserID: "user-1"}, nil)
	...
	assert.Equal(t, model.ExchangeSourceMigration, captured.ExchangeRateSource)
	// Legacy migration must not call FX and must not re-fetch Finance context.
	fxClient.AssertNotCalled(t, "ConvertWithSnapshot", mock.Anything, mock.Anything)
	periodClient.AssertNotCalled(t, "GetPeriodContext", mock.Anything, mock.Anything, mock.Anything, mock.Anything)
}

func TestCreateProRataInstallment_LegacyMigrationCurrencyMismatchRejects(t *testing.T) {
	repo := new(mockExpenseRepository)
	periodClient := new(mockPeriodContextClient)
	fxClient := new(mockFxClient)
	svc := newTestServiceWithFxClock(repo, periodClient, fxClient, time.Date(2026, 6, 1, 0, 0, 0, 0, time.UTC))
